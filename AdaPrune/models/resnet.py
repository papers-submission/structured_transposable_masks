...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes,  stride=1, expansion=1, downsample=None, groups=1, residual_block=None,batch_norm=True):
        super(BasicBlock, self).__init__()
        self.conv1 = conv3x3(inplanes, planes, stride, groups=groups,bias=not batch_norm)
        self.bn1 = depBatchNorm2d(batch_norm,planes)
        self.relu1 = nn.ReLU(inplace=False)
        self.conv2 = conv3x3(planes, expansion * planes, groups=groups,bias=not batch_norm)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes,  stride=1, expansion=4, downsample=None, groups=1, residual_block=None,batch_norm=True):
        super(Bottleneck, self).__init__()
        self.conv1 = nn.Conv2d(
            inplanes, planes, kernel_size=1, bias=not batch_norm)
        self.bn1 = depBatchNorm2d(batch_norm, planes)
        self.conv2 = conv3x3(planes, planes, stride=stride, groups=groups,bias=not batch_norm)
        self.bn2 = depBatchNorm2d(batch_norm, planes)
        self.conv3 = nn.Conv2d(
            planes, planes * expansion, kernel_size=1, bias=not batch_norm)
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, expansion=1, stride=1, groups=1, residual_block=None, batch_norm=True):
        downsample = None
        ...
        layers.append(block(self.inplanes, planes, stride, expansion=expansion,
                            downsample=downsample, groups=groups, residual_block=residual_block,batch_norm=batch_norm))
        self.inplanes = planes * expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, expansion=expansion, groups=groups,
                                residual_block=residual_block,batch_norm=batch_norm))
...
class ResNet_imagenet(ResNet):
    ...
    def __init__(self, num_classes=1000, inplanes=64,
                 ...
                 regime='normal', scale_lr=1,batch_norm=True):
        ...
        for i in range(len(layers)):
            setattr(self, 'layer%s' % str(i + 1),
                    self._make_layer(block=block, planes=width[i], blocks=layers[i], expansion=expansion,
                                     stride=1 if i == 0 else 2, residual_block=residual_block, groups=groups[i],batch_norm=batch_norm))
