...
from vision.utils.meters import AverageMeter, accuracy
from vision.utils.mixup import MixUp, CutMix
...
class Trainer(object):
    ...
    def __init__(self, model, criterion, optimizer=None,
                 ...
                 mixup=None, cutmix=None, loss_scale=1., grad_clip=-1, print_freq=100,record_activations=False):
        self._model = model
    ...
    def config_record_activations(self, enabled):
        if self.record_activations == enabled:
            return
        self.record_activations = enabled
        if self.record_activations:
            self.activations = {}
            for name, module in self.model.named_modules():
                handle = module.register_forward_hook(partial(self.save_activation,name))
                self.record_fwd_hook_handles.append(handle)
        else:
            for handle in self.record_fwd_hook_handles:
                handle.remove()
    ...
    def save_activation(self,name, mod, inp, out):
        if self.epoch in [0,20,30,40,60,80,90] and name not in self.activations:
	        #self.activations[name].append(out.cpu().numpy())
            val = out.detach().cpu().dequantize().numpy() if out.is_quantized else out.detach().cpu().numpy()
            self.activations[name] = val
            #self.activations[name] = inp[0].detach().cpu().shape
    ...
    def calibrate_bn(self, data_loader, num_steps=None):
        ...
        for k, m in self.model.named_modules():
            if isinstance(m, nn.BatchNorm2d) or isinstance(m, nn.BatchNorm1d):
                momentum[k] = m.momentum
                m.momentum = None
                m.track_running_stats = True
                m.reset_running_stats()
        self.model.train()
        with torch.no_grad():
            ret = self.forward(data_loader, num_steps=num_steps, training=False)
        for k, m in self.model.named_modules():
            if isinstance(m, nn.BatchNorm2d) or isinstance(m, nn.BatchNorm1d):
                m.momentum = momentum[k]
        return ret
