...
from vision.data import DataRegime, SampledDataRegime
from vision.utils.log import setup_logging, ResultsLog, save_checkpoint, export_args_namespace
from vision.utils.optim import OptimRegime
from vision.utils.cross_entropy import CrossEntropyLoss
from vision.utils.misc import torch_dtypes
from vision.utils.param_filter import FilterModules, is_bn
from datetime import datetime
from ast import literal_eval
from vision.trainer import Trainer
from prune.sparsity_freezer import SparsityFreezer
...
parser.add_argument('--sparsity-freezer', action='store_true', default=False,
                    help='if true, will freeze sparse zeros in parameters during training')
...
def evaluate(model, args):
    ...
    trainer = Trainer(model, criterion=criterion, optimizer=None,
...
def train_loop(args, model, trainer, optimizer, train_data, val_data, results, best_prec1, prec5_at_best_prec1,
               save_path):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        val_results = trainer.validate(val_data.get_loader(), num_steps=None)
        ...
        results.plot(x='epoch', y=['training loss', 'validation loss'], legend=['training', 'validation'],
                     title='Loss', ylabel='loss')
        results.plot(x='epoch', y=['training error1', 'validation error1'], legend=['training', 'validation'],
                     title='Error@1', ylabel='error %')
        results.plot(x='epoch', y=['training error5', 'validation error5'], legend=['training', 'validation'],
                     title='Error@5', ylabel='error %')
        if 'grad' in train_results.keys():
            results.plot(x='epoch', y=['training grad'], legend=['gradient L2 norm'],
...
def main_worker(args):
    ...
    setup_logging(path.join(save_path, 'log.txt'), resume=args.resume is not '',
    ...
    if args.sparsity_freezer:
        logging.info('Freezing pruned zeros in parameters')
        SparsityFreezer.freeze(model)
    ...
    # train model
    train_loop(args, model, trainer, optimizer, train_data, val_data, results, best_prec1, prec5_at_best_prec1, save_path)
