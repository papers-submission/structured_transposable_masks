...
class ResNet(nn.Module):
    ...
    def features(self, x):
		...
		# modify to overcome GC issue with shape operator
...
class ResNet_imagenet_bfp16(ResNet_imagenet):
    def __init__(self, **config):
        super(ResNet_imagenet_bfp16, self).__init__(**config)
        patched_torch = {'add', 'addmm'}
        patched_nn = {'adaptive_avg_pool2d', 'batch_norm', 'conv2d', 'max_pool2d'}
        self.lowp_config = SimpleNamespace(precision='BF16', warn_patched=False, warn_not_patched=False,
                                           patched_torch=patched_torch, patched_nn=patched_nn)

    def forward(self, x):
        with lowp.Lowp(mode=self.lowp_config.precision,
                       warn_patched=self.lowp_config.warn_patched,
                       warn_not_patched=self.lowp_config.warn_not_patched,
                       patched_torch=self.lowp_config.patched_torch,
                       patched_nn=self.lowp_config.patched_nn):
            return super(ResNet_imagenet_bfp16, self).forward(x)
...
class ResNet_cifar_bfp16(ResNet_cifar):
    def __init__(self, **config):
        super(ResNet_cifar_bfp16, self).__init__(**config)
        patched_torch = {'add', 'addmm'}
        patched_nn = {'adaptive_avg_pool2d', 'batch_norm', 'conv2d', 'max_pool2d'}
        self.lowp_config = SimpleNamespace(precision='BF16', warn_patched=False, warn_not_patched=False,
                                           patched_torch=patched_torch, patched_nn=patched_nn)
    ...
    def forward(self, x):
        with lowp.Lowp(mode=self.lowp_config.precision,
                       ...
                       patched_torch=self.lowp_config.patched_torch,
                       patched_nn=self.lowp_config.patched_nn):
            return super(ResNet_cifar_bfp16, self).forward(x)
...
def resnet(**config):
    ...
    elif config.pop('bfp16', False):
        print('Using BFP16 modules')
        use_bfp16 = True
    ...
    if 'imagenet' in dataset:
        ...
        resnet_class = ResNet_imagenet_bfp16 if use_bfp16 else ResNet_imagenet
        return resnet_class(**config)

    elif dataset in ('cifar10', 'cifar100'):
        n_classes = 10 if dataset == 'cifar10' else 100
        config.setdefault('num_classes', n_classes)
        resnet_class = ResNet_cifar_bfp16 if use_bfp16 else ResNet_cifar
